...
When you open a `.codex` file that has video timestamps (e.g., imported from subtitles), a **Video** tab appears in the cell editor. The video player:
...
Imported and recorded audio is stored in `.project/attachments/files/{documentId}/` within your project. Files are referenced by attachment ID in cell metadata, not by book/chapter/verse naming convention. The importer manages this automatically.
